...
const getStatusColor = (status: string) => {
  if (status === '해결완료') return 'var(--green)';
  if (status === '해결중') return 'var(--orange)';
  return 'var(--red)'; 
};
...
export default function ErrorDetail({ error, onBack }: ErrorDetailProps) {
  ...
  const handleStatusChange = async (displayStatus: string) => {
    try {
      setIsUpdating(true);
      const apiStatus = statusToApi[displayStatus] || 'unresolved';
      ...
      const res = await fetch(`/api/admin/errors/${error.id}/status`, {
        ...
        body: JSON.stringify({ 
          status: apiStatus, 
          resolution: `${displayStatus} 상태로 변경됨` 
        })
      });
  ...
  const handleRetry = async () => {
    try {
      setIsUpdating(true);
      const res = await fetch(`/api/admin/errors/${error.id}/retry`, {
        method: 'POST',
        credentials: 'include',
      });
      if (res.status === 401) { navigate('/'); return; }
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden', marginBottom: '24px' }}>
        ...
        <div style={{ padding: '8px 0' }}>
          {[
            ...
            { label: '구간', value: error.step },
            { label: '대상', value: error.target },
          ].map((row, idx) => (
