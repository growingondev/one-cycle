...
def _validate_collection_run_for_publish(
    ...
    if (
        ...
        raise RuntimeError(
            ...
            "다릅니다. "
            f"declared={collection_run.successful_announcement_count}, "
    ...
    for announcement in announcements:
        ...
        if not documents:
            raise RuntimeError(
                "Document가 없는 Announcement가 있습니다. "
        ...
        for document in documents:
            ...
            if processing_run is None:
                raise RuntimeError(
                    "active ProcessingRun이 없는 Document가 있습니다. "
                    f"document_id={document.id}"
                )
            ...
            chunk_set = db.scalar(
                select(ChunkSet).where(
                    ChunkSet.processing_run_id == processing_run.id,
            ...
            embedding_chunk_count += completed_embedding_chunks
...
def publish_collection_run(
    ...
    with SessionLocal.begin() as db:
        ...
        if previous_collection_run_id == collection_run_id:
            return {
                **summary,
        ...
        system_state.active_collection_run_id = collection_run_id
