...
Worker는 Primary 문서의 실제 내부 형식을 검사하고, 파일명이나 DB 형식과 달라도 판별된 형식에 맞는 HWP 또는 HWPX Parser를 선택합니다. 이후 정규화, 구조화, 검증, 청킹을 수행하고 Embedding Service를 호출한 뒤 핵심정보를 추출합니다. Backend는 결과를 검증해 `ProcessingRun`, `DocumentStructure`, `ChunkSet`, `Chunk`, `Embedding`, `KeyInformation`으로 저장합니다.
...
환경변수와 평가 환경을 포함한 상세 실행 방법은 [Environment](docs/ENVIRONMENT.md)를 참고합니다.
