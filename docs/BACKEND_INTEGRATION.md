...
Backend가 전달하는 `document_format`은 DB에 등록된 형식입니다. Document Worker는 원본 파일의 실제 내부 형식을 다시 검사하고, 판별 결과에 따라 HWP 또는 HWPX Parser를 선택합니다. 실제 형식이 등록 형식과 달라도 Worker는 실제 형식을 응답하며 Backend는 이를 정상 보정 결과로 허용합니다.
