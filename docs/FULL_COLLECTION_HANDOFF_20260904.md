...
- 현재 활성 Run과 직전 정상 Run 1개만 보관하고 더 오래된 Run을 정리하는 기능
- 오래된 Run에 연결된 DB 데이터와 원본·처리 결과 파일의 안전한 삭제
...
현재 “전체교체”는 active CollectionRun 전환이며 과거 데이터를 물리적으로 삭제하지 않는다.
따라서 관리자 API에는 과거 Run의 공고가 함께 조회돼 수백 건으로 보일 수 있다.
