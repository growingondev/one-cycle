...
검증 시각: 2026-09-05 KST (2026-09-04 UTC)

### 적용 상태

- 적용 코드: `develop-api` 병합 커밋 `7b71fb8`
- DB 마이그레이션: `d91f7a63b2c4`
- 보관·삭제 모드: `delete`
- Backend와 Scheduler 재생성 및 정상 실행 확인
- Scheduler 실행 기준: `Asia/Seoul` 12:00, 15:00, 18:00

### 기존 데이터 정리 결과

정리 전 dry-run에서 다음 조건을 확인했다.

- 활성 Run: `10`
- 직전 정상 Run: `9`
- 삭제 후보 Run: `1, 2, 4, 5, 6, 7, 8`
- 실행 중인 삭제 후보 Run: 없음
- legacy 경로 연결 후 위험 경로: 0개

실제 정리 결과는 다음과 같다.

- 삭제한 CollectionRun: 7개
- 삭제한 원본 문서 파일: 521개
- 삭제한 문서 처리 결과 디렉터리: 282개
- 누락된 파일: 0개
- 파일 삭제 오류: 0개
- 보호된 활성 Run: `10`
- 보호된 직전 Run: `9`

AWS의 과거 저장 공간은 다음과 같이 감소했다.

- 과거 문서 저장소: 약 61MB → 1.7MB
- 과거 처리 결과 저장소: 약 2.4GB → 928MB

남은 legacy 파일은 보호 Run이 참조하거나 DB 정리 대상이 아닌 파일일 수
있으므로 추가로 일괄 삭제하지 않았다.

### 남은 운영 검증

다음 정기 전체수집이 성공하면 새 Run이 활성화되고, Run `10`이
`previous_collection_run_id`로 이동하며, 기존 Run `9`가 자동으로
정리되는지 확인한다.

수집·문서 처리·검증 또는 Publish가 실패하는 경우에는 Run `10`과 Run
`9`가 그대로 유지되고 정리가 실행되지 않아야 한다.
